...
- Next steps:
  - Step 7: connect 8 faders to `param_knob0..7`
  - Step 8: analyzer UI and audio readback plumbing
  - Step 9: apply-cue workflow
- Verification:
  - `./gradlew.bat assembleDebug` passes.
  - Earlier device smoke tests confirmed play/pause and preview freeze-on-pause.
  - A fresh physical-device reinstall is still pending because no handset is currently visible to `adb` in this environment.
